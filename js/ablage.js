...
async function loadExistingFiles() {
  ...
  const normalizeContainer = (f) => {
    ...
    // Heuristik: aus DATEINAMEN ableiten (für alte Uploads ohne Container-Metadaten)
    const fname = (f.name ?? f.filename ?? "").toString().toLowerCase().trim();
    if (fname) {
      if (fname.startsWith("wahlvorschlag")) return "wahlvorschlag";
      if (fname.startsWith("wahlausschreiben")) return "wahlausschreiben";
      if (fname.startsWith("niederschrift")) return "niederschrift";
      if (fname.startsWith("bekanntmachung")) return "bekanntmachung";
      // auch Treffer irgendwo im Namen zulassen (z.B. "bkz123_wahlvorschlag_...")
      if (fname.includes("wahlvorschlag")) return "wahlvorschlag";
      if (fname.includes("wahlausschreiben")) return "wahlausschreiben";
      if (fname.includes("niederschrift")) return "niederschrift";
      if (fname.includes("bekanntmachung")) return "bekanntmachung";
    }
